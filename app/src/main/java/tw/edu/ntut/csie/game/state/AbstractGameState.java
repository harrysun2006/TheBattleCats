...
public abstract class AbstractGameState extends GameState {
    ...
    public boolean pointerPressed(List<Pointer> pointers) {
        return notifyPointerPressedEvent(pointers);
    }

    @Override
    public boolean pointerMoved(List<Pointer> pointers) {
        return notifyPointerMovedEvent(pointers);
    }

    @Override
    public boolean pointerReleased(List<Pointer> pointers) {
        return notifyPointerReleasedEvent(pointers);
    }
     ...
     */
    protected boolean notifyPointerPressedEvent(List<Pointer> pointers) {
        for (PointerEventHandler handler : _pointerHandlers) {
            if (handler.pointerPressed(pointers)) {
                return true;
            }
        }
        return false;
     ...
     * @return true表示該事件已經被處理完畢，不用再繼續傳遞下去。
     */
    protected boolean notifyPointerMovedEvent(List<Pointer> pointers) {
        for (PointerEventHandler handler : _pointerHandlers) {
            if (handler.pointerMoved(pointers)) {
                return true;
            }
        }
        return false;
     ...
     * @return true表示該事件已經被處理完畢，不用再繼續傳遞下去。
     */
    protected boolean notifyPointerReleasedEvent(List<Pointer> pointers) {
        for (PointerEventHandler handler : _pointerHandlers) {
            if (handler.pointerReleased(pointers)) {
                return true;
            }
        }
        return false;
